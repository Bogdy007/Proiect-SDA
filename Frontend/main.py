...
@app.route('/api/interventii/update/<int:id_interventie>', methods=['PUT'])
def update_interventie(id_interventie):
    d = request.json
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
                sql = """UPDATE Interventii \
                         SET NR_INVENTAR=%s, \
                             DATA_INTERVENTIE=%s, \
                             TIP_INTERVENTIE=%s, \
                             TIP_OPERATIE=%s, \
                             DESCRIERE_INTERVENTIE=%s, \
                             componente_schimbate_adaugate=%s, \
                             DURATA_INTERVENTIE=%s, \
                             OPERATOR=%s, \
                             OBSERVATII=%s
                         WHERE ID_INTERVENTIE = %s"""
                val = (d.get('NR_INVENTAR'), d.get('DATA_INTERVENTIE') or None, d.get('TIP_INTERVENTIE'),
                       d.get('TIP_OPERATIE'),
                       d.get('DESCRIERE_INTERVENTIE'), d.get('componente_schimbate_adaugate'),
                       d.get('DURATA_INTERVENTIE'), d.get('OPERATOR'), d.get('OBSERVATII'),
                       id_interventie)
                cursor.execute(sql, val)
                conn.commit()
        return jsonify({"succes": True}), 200
...
@app.route('/api/interventii/delete/<int:id_interventie>', methods=['POST'])
def delete_interventie(id_interventie):
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute("DELETE FROM Interventii WHERE ID_INTERVENTIE = %s", (id_interventie,))
                conn.commit()
        return jsonify({"succes": True}), 200
    except Exception as e:
        return jsonify({"succes": False, "eroare": str(e)}), 500
...
def print_pdf(nr_inventar):
    try:
        ...
        w_cols = [18, 22, 22, 35, 30, 15, 20, 28]
        headers = ['Data', 'Tip', 'Op.', 'Descriere', 'Comp.', 'Timp', 'Operator', 'Obs.']
        ...
        if not interventii:
            ...
            for row in interventii:
                ...
                op_tip = str(row.get('TIP_OPERATIE') or '-')
                desc = str(row.get('DESCRIERE_INTERVENTIE') or '-')
                comp = str(row.get('componente_schimbate_adaugate') or '-')
                durata = str(row.get('DURATA_INTERVENTIE') or '-')
                operator = str(row.get('OPERATOR') or '-')
                obs = str(row.get('OBSERVATII') or '-')

                pdf.cell(w_cols[0], 8, data[:12], 1, 0, 'C', fill)
                pdf.cell(w_cols[1], 8, tip[:15], 1, 0, 'L', fill)
                pdf.cell(w_cols[2], 8, op_tip[:15], 1, 0, 'L', fill)
                pdf.cell(w_cols[3], 8, desc[:25], 1, 0, 'L', fill)
                pdf.cell(w_cols[4], 8, comp[:20], 1, 0, 'L', fill)
                pdf.cell(w_cols[5], 8, durata[:10], 1, 0, 'C', fill)
                pdf.cell(w_cols[6], 8, operator[:15], 1, 0, 'C', fill)
                pdf.cell(w_cols[7], 8, obs[:20], 1, 1, 'L', fill)
